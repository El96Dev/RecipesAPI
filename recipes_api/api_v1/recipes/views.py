...
async def create_recipy(
    ...
    if not await crud.check_cuisine_exists(session, recipy_in.cuisine):
        raise HTTPException(
            status_code=422,
            detail=f"Cuisine {recipy_in.cuisine} doesn't exist!"
        )
    if not await crud.check_category_exists(session, recipy_in.category):
        raise HTTPException(
            status_code=422,
            detail=f"Category {recipy_in.category} doesn't exist!"
        )
    else:
        return await crud.create_recipy(
            session=session, recipy_in=recipy_in, author_id=user.id
        )
...
async def delete_recipy(
    recipy_id: int,
    session: AsyncSession = Depends(db_helper.scoped_session_dependency),
    user: User = Depends(current_active_user)
):
    recipy = await get_recipy_if_user_is_author(
        recipy_id=recipy_id,
        author_id=user.id,
        session=session
    )
...
async def add_like(
    ...
    if await crud.check_if_like_exists(
        session=session, user_id=user.id, recipy_id=recipy_id
    ):
        raise HTTPException(
            status_code=403,
            detail="Like already exists!"
        )
    elif not await crud.check_if_recipy_exists(
        session=session, recipy_id=recipy_id
    ):
        raise HTTPException(
            status_code=404,
            detail=f"Recipe with id {recipy_id} doesn't exist!"
        )
    return await crud.add_like(
        session=session, user_id=user.id, recipy_id=recipy_id
    )
...
async def delete_like(
    ...
    if not await crud.check_if_like_exists(
        session=session, user_id=user.id, recipy_id=recipy_id
    ):
        raise HTTPException(status_code=404, detail="Like doesn't exits!")
    like = await crud.get_like(
        session=session, user_id=user.id, recipy_id=recipy_id
    )
...
async def set_recipy_image(
    ...
    await crud.set_recipy_image(
        recipy_id, recipy_image, user, session
    )
